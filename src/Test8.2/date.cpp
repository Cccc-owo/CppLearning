...
int Date::daysTo(int ayear, int amonth, int aday) const {
	int y2, m2, d2;
	int y1, m1, d1;
	...
	m2 = (amonth + 9) % 12;
	y2 = ayear - m2 / 10;
	d2 = 365 * y2 + y2 / 4 - y2 / 100 + y2 / 400 + (m2 * 306 + 5) / 10 + (aday - 1);

	return (d2 - d1);
}

int Date::operator-(const Date& date2) const {
	return daysTo(date2.getYear(), date2.getMonth(), date2.getDay());
}
